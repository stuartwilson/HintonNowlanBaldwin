...
double getFit(std::vector<int> x, int L, int U, double f_min, vector<int> target){
    ...
    if(c){
        ...
        double Plower = 1.0-pow((1.0-pow(0.5,c)),L);
        double Pupper = 1.0-pow((1.0-pow(0.5,c)),U);
        double r = rng->get();
...
int main (int argc, char **argv){
    ...
    if (argc < 3) { std::cerr << "\nUsage: ./evodevo configfile logdir cutoff seed(optional)"; return -1; }
    ...
    for(int g=0;g<G;g++){
        ...
        double sumF = 0.;
        for(int p=0;p<P;p++){
            F[p] = getFit(X[p],L,U,f_min,target);
            sumF += F[p];
        }
